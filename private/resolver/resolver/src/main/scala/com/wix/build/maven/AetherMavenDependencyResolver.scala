...
class AetherMavenDependencyResolver(remoteRepoURLs: => List[String],
  ...
  private val artifactDescriptorStore = mutable
    .Map[Artifact, ArtifactDescriptorResult]()
    .withDefault(artifactDescriptorOf)
  ...
  private def maybeVersionFromPom(project: Model) = Option(project.getVersion)

  private def maybeVersionFromParent(project: Model) = Option(project.getParent).map(_.getVersion)
  ...
  private def fromAetherDependencyNode(node: AetherDependencyNode): DependencyNode = DependencyNode(
