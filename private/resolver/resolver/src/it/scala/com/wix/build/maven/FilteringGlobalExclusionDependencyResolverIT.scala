...
import MavenMakers.{randomCoordinates, randomDependency}
...
class FilteringGlobalExclusionDependencyResolverIT
  ...
  "FilteringGlobalExclusionDependencyResolver" should {
    "keep managed dependencies when retaining transitive dependencies" in {
      ...
      filteringGlobalExclusionDependencyResolver
        .directDependenciesOf(interestingArtifact.coordinates) must contain(exactly(secondLevelTransitiveDependency))
    }
  }
}
