...
import DefaultChecksumValues.{defaultChecksum, defaultSrcChecksum}
...
object MavenMakers {
  ...
  def someArtifactId(artifactIdPrefix: String = defaultArtifactPrefix,
                     index: Int = Random.nextInt): String = artifactIdPrefix + index

  def randomCoordinates(withVersion: String = randomString(),
                        artifactIdPrefix: String = defaultArtifactPrefix,
                        index: Int = Random.nextInt()): Coordinates =
    Coordinates(someGroupId, someArtifactId(artifactIdPrefix, index), withVersion)

  def randomDependency(withVersion: String = randomString(),
  ...
  def someProtoCoordinates(artifactId: String): Coordinates =
    Coordinates("some.group", artifactId, "some-version", packaging = Packaging("zip"), classifier = Some("proto"))
  ...
  def aTestArchiveZipDependency(artifactId: String): Dependency = Dependency(someCoordinates(artifactId)
    .copy(packaging = Packaging("zip")), MavenScope.Test)

  def aRootDependencyNode(dependency: Dependency): DependencyNode = DependencyNode(dependency, Set.empty)

  def aRootBazelDependencyNode(dependency: Dependency,
                               checksum: Option[String] = Some(defaultChecksum),
                               srcChecksum: Option[String] = Some(defaultSrcChecksum)): BazelDependencyNode =
    BazelDependencyNode(dependency, Set.empty, checksum = checksum, srcChecksum = srcChecksum)

  def aRootBazelSnapshotDependencyNode(dependency: Dependency): BazelDependencyNode =
    BazelDependencyNode(dependency, Set.empty, snapshotSources = true)

  def aBazelDependencyNode(dependency: Dependency, dependencies: Set[Dependency]): BazelDependencyNode =
    BazelDependencyNode(dependency, dependencies, Some(defaultChecksum), Some(defaultSrcChecksum))

  def dependencyNodesFrom(singleDependency: SingleDependency): Set[DependencyNode] = Set(
    DependencyNode(singleDependency.dependant, Set(singleDependency.dependency)),
    DependencyNode(singleDependency.dependency, Set())
  )
}
