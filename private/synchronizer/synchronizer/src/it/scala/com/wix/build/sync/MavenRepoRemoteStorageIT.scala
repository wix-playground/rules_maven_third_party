...
import com.wix.build.sync.FakeWireMockMavenRepoTestSupport._
...
class MavenRepoRemoteStorageIT extends SpecificationWithJUnit with BeforeAll with AfterAll with BeforeEach {
  ...
  "MavenRepoRemoteStorage" should {
    "first try to download the sha256 directly" in {
      val storage = new MavenRepoRemoteStorage(List(s"http://localhost:$wireMockPort"), new VoidArtifactsShaCache)
    ...
    "download jar from second url and calculate sha256" in {
      val storage = new MavenRepoRemoteStorage(List(s"http://localhost:$wireMockPort", s"http://localhost:$secondWireMockPort"), new VoidArtifactsShaCache)
  ...
  override def beforeAll = {
    ...
    secondWireMockServer.start()
  }

  override def afterAll(): Unit = {
    wireMockServer.stop();
    secondWireMockServer.stop()
  }
}
